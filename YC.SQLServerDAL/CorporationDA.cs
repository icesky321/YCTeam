...
namespace YC.SQLServerDAL
{
    public class CorporationDA : IDisposable
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IQueryable<YC.SQLServerDAL.Corporation> GetCorporation()
        {
            var query = from c in dbContext.Corporation
                        select c;
            return query.AsQueryable<YC.SQLServerDAL.Corporation>();
        }
        ...
        /// <returns></returns>
        public YC.SQLServerDAL.Corporation GetCorporationById(Guid corpId)
        {
            var query = from c in dbContext.Corporation
                        where c.CorpId == corpId
                        select c;
            return query.FirstOrDefault<YC.SQLServerDAL.Corporation>();
        }
        ...
        /// <returns></returns>
        public YC.SQLServerDAL.Corporation GetCorporationByName(string corpName)
        {
            var query = from c in dbContext.Corporation
        ...
        public void UpdateCorporation(YC.SQLServerDAL.Corporation corp)
        {
            dbContext.SubmitChanges();
        }
